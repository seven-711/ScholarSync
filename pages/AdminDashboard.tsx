...
import { Users, BookOpen, AlertCircle, CheckCircle2, Clock, FileText, Check, X, Activity, Shield, AlertTriangle, UserX, Loader2, IdCard, Building, Mail, Calendar, GraduationCap } from 'lucide-react';
...
export const AdminDashboard: React.FC<Props> = ({ onUpdate }) => {
  ...
  const [recentPending, setRecentPending] = useState<Profile[]>([]);
  ...
  const totalUsers = stats.activeScholars + stats.pendingScholars;
      ...
      {stats.pendingScholars > 0 && (
        <div className="bg-orange-50 border-l-4 border-orange-500 p-4 md:p-6 rounded-r-xl shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center gap-4 w-full">
            ...
            <div>
              <h3 className="text-lg font-bold text-orange-900">Action Required: {stats.pendingScholars} Pending</h3>
              <p className="text-orange-700 text-sm">New scholars waiting for review.</p>
            </div>
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 shadow-sm relative overflow-hidden group">
           ...
           <div className="relative z-10">
             ...
             <p className="text-4xl font-extrabold text-amber-900">{stats.pendingScholars}</p>
          ...
          <div id="pending-table" className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            <div className="p-5 border-b border-gray-200 bg-gray-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">Manage scholar access requests</p>
              </div>
              {stats.pendingScholars > 0 && (
                    ...
                    {recentPending.map((scholar) => (
                      <tr key={scholar.id} className="hover:bg-amber-50/30 transition-colors">
                        <td className="px-6 py-4">
                           ...
                           <div className="font-mono text-gray-500 text-xs sm:hidden mt-1">{scholar.email}</div>
                        </td>
                        <td className="px-6 py-4 font-mono text-gray-600 text-xs hidden sm:table-cell">{scholar.email}</td>
                        <td className="px-6 py-4">
