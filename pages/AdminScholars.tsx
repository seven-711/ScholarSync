...
export const AdminScholars: React.FC<Props> = ({ profile, onUpdate }) => {
  ...
  const fetchScholars = async () => {
    ...
    // Base Queries
    let pendingQuery = supabase.from('profiles').select('*').eq('role', 'scholar').eq('is_approved', false).order('created_at', { ascending: false });
    let activeQuery = supabase.from('profiles').select('*').eq('role', 'scholar').eq('is_approved', true).order('full_name', { ascending: true });

    // SCOPE FILTER: If not Super Admin, filter by school
    if (profile?.role !== UserRole.SUPER_ADMIN && profile?.school) {
      pendingQuery = pendingQuery.eq('school', profile.school);
